...
setClass(
  ...
  prototype(logCounts = NULL, t = NULL, w = NULL)
)
...
setValidity("lineageDEDataSet", function(object){
  ...
  if(!is.numeric(X)) {
    return('logCounts coordinates must be numeric.')
  }
  if(n==0){
    return('logCounts has zero rows.')
  }
  if(m==0){
    return('logCounts has zero columns.')
  }
  if (nrow(t(object))!= m){
  ...
  if(is.null(rownames(X))){
    rownames(logCounts(object)) <- paste('gene',
                                          seq_len(nrow(X)),
                                          sep='-')
  }
  if(is.null(colnames(X))){
    colnames(logCounts(object)) <- paste('Cell',
                                          seq_len(ncol(X)),
                                          sep='-')
  }
