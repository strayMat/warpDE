...
load("tests/test.data.RData")
load("~/Documents/Berkeley/work_Berkeley/Lineage_TE/rankings/vgam.rank.RDATA")
```
...
#save(df, file = "~/Documents/Berkeley/work_Berkeley/lineageDE/tests/data_tests.RDATA")
```
...
The comparision between the alternative model and the null model can be computed via likelihood ratio test or with AIC. The function \code{likelihood_rank} computes these values and returns a ranking.
```{r}
lkl.rank <- likelihood_rank(df, reg = "vgam")
aic.ranking <- lkl.rank$aic
pval.ranking <- lkl.rank$pval

#save(aic.ranking, pval.ranking, file = "~/Documents/Berkeley/work_Berkeley/Lineage_TE/rankings/vgam.rank.RDATA")
ranking.df(pval.ranking)
plot_multigenes(df.t, aic.ranking, rownames(ranking.df(aic.ranking))[1:9], grid.size = c(2,4))
### does not work well; ask sandrine!!!!
plot_multigenes(df.t, pval.ranking, tail(rownames(ranking.df(pval.ranking)), 9), grid.size = c(2,4))
