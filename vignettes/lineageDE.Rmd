...
```{r setup, include = F}
knitr::opts_chunk$set(cache=TRUE, message = F, warning = F, fig.height = 4, fig.width = 8, fig.align = "center")
library(ggplot2)
...
w <- cbind(w1,w2)
df <- new("lineageDEDataSet", counts = f.counts, t = times, w = w)
```

The other dataset in the package `lineageDEDataSet_example` contains exactly this `df` object, so you are not forced to redo the whole Slinghshot workflows.
...
```{r, include=T, eval =F}
s.mix.ranking <- likelihood_rank(df, reg.f = "ns", s.df = 4, dtw = T)$aic
```
...
```{r, fig.width= 7}
dtw.aic <- rankings_compare(s.dtw.ranking, s.mix.ranking)
dtw.aic$pl
dtw.aic$shared.q
...
kendall.heatmap(list(s.aic.ranking, s.mix.ranking, s.dtw.ranking, euclidean.ranking))
```
