...
```{r setup, include = F}
knitr::opts_chunk$set(cache=TRUE, message = F, warning = F, fig.height = 6, fig.width = 10, fig.align = "center")
library(ggplot2)
...
The other dataset in the package `lineageDEDataSet_example` contains exactly this `df` object, so you are not forced to re-do the whole Slinghshot workflow.
...
load("/home/matthieu/Documents/Berkeley/work_Berkeley/lineageDE/tests/f.rankings/s.lkl.ranks.RData")
load("/home/matthieu/Documents/Berkeley/work_Berkeley/lineageDE/tests/f.rankings/s.dtw.ranks.RData")
load("/home/matthieu/Documents/Berkeley/work_Berkeley/lineageDE/tests/f.rankings/s.mix.ranks.RData")
```
...
s.aic.ranks <- s.rank$aic
s.pval.ranks <- s.rank$pval
```

You can then  plot the first genes picked by the method by running `plot_multigenes`
```{r}
plot_multigenes(df, s.aic.ranks, rownames(ranking.df(s.aic.ranks))[1:8], grid.size = c(2,4))
plot_multigenes(df, s.pval.ranks, rownames(ranking.df(s.pval.ranks))[1:8], grid.size = c(2,4))
```

Here is also an exemple of the worst genes picked by this method : 
```{r}
plot_multigenes(df, s.pval.ranks, tail(rownames(ranking.df(s.pval.ranks)),8), grid.size = c(2,4))
```
...
```{r, include = T, eval = F}
s.dtw.ranks <- dtw_rank(df, reg.f = "ns", s.df = 4)
```

```{r}
plot_multigenes(df, s.dtw.ranks, rownames(ranking.df(s.dtw.ranks))[1:8], grid.size = c(2,4))
```
...
```{r, include=T, eval =F}
s.mix.ranks <- likelihood_rank(df, reg.f = "ns", s.df = 4, dtw = T)$aic
```

```{r, fig.align = 'center'}
plot_multigenes(df, s.mix.ranks, rownames(ranking.df(s.mix.ranks))[1:8], grid.size = c(2,4))
```
...
```{r, fig.width= 7}
dtw.aic <- rankings_compare(s.dtw.ranks, s.mix.ranks)
dtw.aic$pl
dtw.aic$shared.q
...
kendall.heatmap(list(s.aic.ranks, s.mix.ranks, s.dtw.ranks))
```
