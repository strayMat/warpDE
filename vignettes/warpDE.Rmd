...
This vignette uses the data from [@Fletcher2017], examining the differentiation of horizontal basel cells (HBC) into neuronal and sustentatucal cells in the mouse olfactory epithelium (OE). For the sake of simplicity, we restricted the data to only two cell lineages.
...
Before searching for differentially expressed genes between lineages, we have to preprocess the data and run `slingshot` on it. We use data from [@Fletcher2017] [*** SD: Say where the data can be obtained.], that were already filtered and normalized with [`scone`](https://github.com/YosefLab/scone). The R package `scone` implements a variety of normalization procedures, aiming to adjust for technical effects such as batch effects and sequencing depth, and provides performance measures for selecting an appropriate procedure. 
