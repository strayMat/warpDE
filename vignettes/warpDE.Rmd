...
This vignette describes the workflow of the R package [`warpDE`](https://github.com/strayMat/warpDE), which provides three different methods to infer differentially expressed (DE) genes from reconstructed cell lineages. After a first step of lineage inference and cell pseudotime ordering performed with the R package [`slingshot`](https://github.com/kstreet13/slingshot) [@Street2017], the goal is to identify genes that are DE between two cell lineages.

This vignette uses data from [@Fletcher2017], examining the differentiation of horizontal basel cells (HBC) into neuronal cells, via globose basal cells (GBC), and into sustentatucal cells, in the mouse olfactory epithelium (OE). For the sake of simplicity, we restricted the data to only two cell lineages.



<img src="images/fig_lignees_study.png" alt="alt text" width=100% height=30%><center>Cell lineages used in the study</center>

## 1.1 - Preprocessing

Before searching for differentially expressed genes between lineages, we have to preprocess the data and run `slingshot` on it. We use data from [@Fletcher2017], that were already filtered and normalized with [`scone`](https://github.com/YosefLab/scone) (The original RNA-seq data can be accessed with the accession number GEO: GSE95601).
Software. The R package `scone` implements a variety of normalization procedures, aiming to adjust for technical effects such as batch effects and sequencing depth, and provides performance measures for selecting an appropriate normalization. 

The preprocessed OE data are available in the package `warpDE`, which contains also the different functions that we use in this study. We also load the package `slingshot` to perform lineage and pseudotime inferrence.
...
We perform a usual transformation with `log(1+x)` and we apply principal component analysis (PCA) as the dimensionality reduction technique:
```{r}
...
We can also have a 2D plot of the lineages by projecting onto the first two PCs:
...
Then, we store the `slingshot` output in a new data object of class `warpDEDataSet` which contains the desired input for our future DE analysis: 
```{r}
...
w <- cbind(w1,w2)
df <- warpDEDataSet(counts, times, w)
```
...
We plot the gene expression profile for one random gene of the dataset (CreER) against cell pseudotimes for each lineage.
...
```
We fit a separate natural spline regression (`ns`) for each lineage, corresponding to the alternative model of distinct pseudotemporal expression patterns, and one `ns` regression including both lineages at the same time, corresponding to the null model of identical pseudotemporal expression patterns. This gives us a representation of the activation pattern for a given gene.
```{r,fig.height=4, fig.width=6, fig.align="center"}
